          ...
          <div style={{ background:'#fff', borderRadius:16, width:'100%', maxWidth:700, maxHeight:'85vh', overflow:'hidden', display:'flex', flexDirection:'column', boxShadow:'0 20px 60px rgba(0,0,0,0.3)' }} onClick={e=>e.stopPropagation()}>
            <div style={{ padding:'20px 24px', borderBottom:`1px solid ${B.border}`, display:'flex', justifyContent:'space-between', alignItems:'center', background:`linear-gradient(135deg,${B.darkRed},${B.red})` }}>
              <div>
                ...
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.75)', marginTop:2 }}>Past-dated visits not yet marked completed — each requires follow-up with the clinician</div>
              </div>
            ...
            <div style={{ padding:'12px 24px', background:'#FEF2F2', borderBottom:'1px solid #FECACA', display:'flex', gap:24, flexWrap:'wrap' }}>
              {[
                ...
                { label:'Clinicians', value:[...new Set((csvData?.missedVisitDetails||[]).map(v=>v.staff))].length, color:B.yellow },
              ].map(s => (
                <div key={s.label}>
              ...
              <div style={{ marginLeft:'auto', fontSize:12, color:B.danger, alignSelf:'center', fontWeight:600 }}>
                ~${((csvData?.missedVisitDetails?.length||0) * (CFG?.avgReimbursement||90)).toLocaleString()} revenue impact
              </div>
            </div>
            <div style={{ overflowY:'auto', flex:1 }}>
              ...
              {!(csvData?.missedVisitDetails?.length) ? (
              ...
              ) : (csvData?.missedVisitDetails||[]).sort((a,b)=>new Date(a.date)-new Date(b.date)).map((v,i) => {
                ...
                return (
                  <div key={i} style={{ display:'grid', gridTemplateColumns:'180px 180px 70px 100px 1fr', padding:'11px 24px', borderBottom:'1px solid #FAF4F2', alignItems:'center', background:isYesterday?'#FFF5F2':'transparent' }}>
                    ...
                    <div style={{ fontSize:12, color:B.gray, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{v.staff}</div>
                    ...
                    <div style={{ fontSize:11, color:isYesterday?B.danger:B.gray, fontWeight:isYesterday?700:400 }}>
                      {dateStr}{isYesterday && <div style={{ fontSize:9, color:B.danger }}>YESTERDAY</div>}
                    </div>
                    <div style={{ fontSize:11, color:B.lightGray, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{(v.event||'').replace(/\*e\*/g,'').trim()}</div>
                  </div>
                );
              })}
            </div>
