...
function parseParioxCSV(text) {
  ...
  const dedupedScheduled=Object.keys(visitDedupeMap).length;
  const dedupedCompleted=Object.values(visitDedupeMap).filter(v=>v.completed).length;
  const staffStats={};
...
function CoordRow({ report, coordinator }) {
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'180px 80px 80px 80px 80px 80px 80px 80px 1fr', padding:'14px 20px', borderBottom:`1px solid ${B.border}`, alignItems:'center' }}>
      ...
      {[{val:caseload,color:caseloadColor},{val:report?.visits_scheduled||0,color:B.black},{val:report?.visits_completed||0,color:B.green},{val:report?.visits_missed||0,color:(report?.visits_missed||0)>3?B.danger:B.yellow},{val:report?.auths_expiring_7d||0,color:(report?.auths_expiring_7d||0)>2?B.danger:B.yellow},{val:report?.new_referrals||0,color:B.darkRed},{val:report?.tasks_open||0,color:(report?.tasks_open||0)>8?B.danger:B.orange}].map((cell,i)=>(
      ...
      <div style={{ paddingLeft:16 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <div style={{ flex:1, height:4, background:'#F5EDEB', borderRadius:2 }}><div style={{ height:'100%', width:`${cr}%`, borderRadius:2, background:cr>85?B.green:cr>70?B.yellow:B.red }} /></div>
...
function CSVUploadPanel({ onDataLoaded, csvData }) {
  ...
  function processRows(rows,headersArr,statusIdx,dateIdx) {
    ...
    const xDedupedSched=Object.keys(xlsxDedupeMap).length;
    const xDedupedComp=Object.values(xlsxDedupeMap).filter(v=>v.completed).length;
      ...
      reader.onload=(e)=>{
        withXLSX((XLSX)=>{
          try {
            ...
            const statusIdx=headers.findIndex(h=>h.includes('status'));
            const dateIdx=headers.findIndex(h=>h==='date');
            if (statusIdx===-1) { setError('Could not find Status column.'); setProcessing(false); return; }
    ...
    <div style={{ background:B.cardBg, border:`1px solid ${B.border}`, borderRadius:16, padding:'24px', boxShadow:'0 1px 4px rgba(139,26,16,0.06)', marginBottom:20 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:B.gray }}>Upload CSV or XLSX — syncs visit schedule to all users via Supabase</div>
        </div>
        {csvData&&<div style={{ background:'#F0FDF4', border:'1px solid #BBF7D0', borderRadius:8, padding:'6px 12px', fontSize:11, color:B.green, fontWeight:600 }}>✓ {csvData.rowCount} records loaded{lastFile?` · ${lastFile}`:''}</div>}
      </div>
      <div onDragOver={e=>{e.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)}
        onDrop={e=>{e.preventDefault();setDragging(false);handleFile(e.dataTransfer.files[0]);}}
        onClick={()=>fileRef.current.click()}
        style={{ border:`2px dashed ${dragging?B.red:'#E8D5D0'}`, borderRadius:12, padding:'28px 20px', textAlign:'center', cursor:'pointer', background:dragging?'#FFF5F2':'#FDFAF9' }}>
        <input ref={fileRef} type="file" accept=".csv,.xlsx,.xls" style={{ display:'none' }} onChange={e=>{handleFile(e.target.files[0]);e.target.value='';}} />
        <div style={{ fontSize:28, marginBottom:8 }}>{processing?'⏳':csvData?'🔄':'📁'}</div>
        <div style={{ fontSize:13, fontWeight:600, color:B.black, marginBottom:4 }}>{processing?'Processing...':csvData?'Upload new file to override':'Drop your Pariox export here'}</div>
        <div style={{ fontSize:11, color:B.lightGray }}>CSV or XLSX · syncs visit schedule to all team members automatically</div>
      </div>
      ...
      {csvData&&(
        <div style={{ marginTop:16, display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
          {[{label:'Completed',value:csvData.completedVisits,color:B.red},{label:'Scheduled',value:csvData.scheduledVisits,color:B.black},{label:'Missed',value:csvData.missedVisits,color:B.danger},{label:'Completion %',value:csvData.scheduledVisits>0?`${Math.round(csvData.completedVisits/csvData.scheduledVisits*100)}%`:'—',color:B.green}].map(m=>(
      ...
      reader.onload=(e)=>{
        withXLSX((XLSX)=>{
          try {
            const wb=XLSX.read(new Uint8Array(e.target.result),{type:'array',cellDates:true});
            const ws=wb.Sheets[wb.SheetNames[0]];
            const result=parseCensusFile(XLSX.utils.sheet_to_csv(ws));
            if (!result) { setError('Could not detect Status column.'); setProcessing(false); return; }
    ...
    <div style={{ background:'#fff', border:'1px solid #F0E4E0', borderRadius:16, padding:'24px', boxShadow:'0 1px 4px rgba(139,26,16,0.06)', marginBottom:20 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:'#8B6B64' }}>Upload Pariox patient census — syncs to all users in real time</div>
        </div>
        {censusData&&<div style={{ background:'#F0FDF4', border:'1px solid #BBF7D0', borderRadius:8, padding:'6px 12px', fontSize:11, color:'#2E7D32', fontWeight:600 }}>✓ {totalCensus} patients · {censusData.lastUpdated}</div>}
      </div>
      <div onDragOver={e=>{e.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)}
        onDrop={e=>{e.preventDefault();setDragging(false);handleFile(e.dataTransfer.files[0]);}}
        onClick={()=>fileRef.current.click()}
        style={{ border:`2px dashed ${dragging?'#D94F2B':'#E8D5D0'}`, borderRadius:12, padding:'24px 20px', textAlign:'center', cursor:'pointer', background:dragging?'#FFF5F2':'#FDFAF9' }}>
        <input ref={fileRef} type="file" accept=".csv,.xlsx,.xls" style={{ display:'none' }} onChange={e=>{handleFile(e.target.files[0]);e.target.value='';}} />
            ...
            {Object.entries(STATUS_META).map(([key,meta])=>{
              ...
              return (
                <div key={key} style={{ background:'#FBF7F6', borderRadius:8, padding:'10px 14px', display:'flex', justifyContent:'space-between', alignItems:'center', border:'1px solid #F0E4E0' }}>
                  ...
                  <div><span style={{ fontSize:18, fontWeight:800, color:meta.color, fontFamily:"'DM Mono',monospace" }}>{count}</span><span style={{ fontSize:10, color:'#BBA8A4', marginLeft:4 }}>{pct}%</span></div>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div style={{ background:B.cardBg, border:`1px solid ${B.border}`, borderRadius:16, padding:'24px', boxShadow:'0 1px 4px rgba(139,26,16,0.06)' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
        <div><div style={{ fontSize:15, fontWeight:700, color:B.black, marginBottom:3 }}>📂 Google Drive Reports</div><div style={{ fontSize:12, color:B.gray }}>Link your Google Sheets or Docs for quick access</div></div>
      ...
      {adding&&(
        <div style={{ background:'#FBF7F6', border:`1px solid ${B.border}`, borderRadius:10, padding:'16px', marginBottom:16 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr auto', gap:10, alignItems:'end' }}>
            <div><label style={{ display:'block', fontSize:11, color:B.gray, textTransform:'uppercase', marginBottom:6, fontWeight:600 }}>Label</label><input value={newLink.label} onChange={e=>setNewLink(p=>({...p,label:e.target.value}))} placeholder="e.g. Gypsy Weekly Report" style={inputStyle} /></div>
            <div><label style={{ display:'block', fontSize:11, color:B.gray, textTransform:'uppercase', marginBottom:6, fontWeight:600 }}>Google Drive URL</label><input value={newLink.url} onChange={e=>setNewLink(p=>({...p,url:e.target.value}))} placeholder="Paste Google Sheets, Doc, or Drive folder URL" style={inputStyle} /></div>
            <button onClick={()=>{if (!newLink.label||!newLink.url) return; onAddLink({...newLink,id:Date.now(),type:getType(newLink.url)}); setNewLink({label:'',url:''}); setAdding(false);}} style={{ background:B.green, border:'none', borderRadius:8, color:'#fff', padding:'8px 16px', fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit', height:38 }}>Add</button>
          </div>
        </div>
      )}
      {driveLinks.length===0&&!adding&&<div style={{ textAlign:'center', padding:'28px', color:B.lightGray, fontSize:13, background:'#FDFAF9', borderRadius:10, border:`1px dashed ${B.border}` }}>No reports linked yet</div>}
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {driveLinks.map(link=>(
          <div key={link.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', background:'#FBF7F6', border:`1px solid ${B.border}`, borderRadius:10 }}>
            ...
            <div style={{ flex:1 }}><div style={{ fontSize:13, fontWeight:600, color:B.black }}>{link.label}</div></div>
            <a href={link.url} target="_blank" rel="noopener noreferrer" style={{ background:'#FFF5F2', border:`1px solid #FDDDD5`, borderRadius:8, color:B.red, padding:'6px 12px', fontSize:12, fontWeight:600, textDecoration:'none' }}>Open →</a>
...
export default function DirectorDashboard({ initialTab='overview', readOnly=false }) {
  ...
  const [expansionData,setExpansionData]=useState(()=>{try{const s=localStorage.getItem('axiom_expansion');if(s)return JSON.parse(s);}catch{}return{GA:{state:'Georgia',status:'In Progress',credentialing:60,staffHired:2,staffNeeded:4,firstPatientDate:'2026-05-01',weeklyVisitTarget:80,currentVisits:0,notes:''},TX:{state:'Texas',status:'Planning',credentialing:20,staffHired:0,staffNeeded:6,firstPatientDate:'2026-07-01',weeklyVisitTarget:120,currentVisits:0,notes:''},NC:{state:'North Carolina',status:'Planning',credentialing:10,staffHired:0,staffNeeded:3,firstPatientDate:'2026-08-01',weeklyVisitTarget:60,currentVisits:0,notes:''}};});
  ...
  const [settings,setSettings]=useState(()=>{try{const s=localStorage.getItem('axiom_settings');return s?JSON.parse(s):DEFAULT_SETTINGS;}catch{return DEFAULT_SETTINGS;}});
  ...
  const handleCSV = async data => {
    ...
    if (data.staffList?.length > 0) {
      setStaffDirectory(prev => {
        ...
        (data.staffList || []).forEach(s => {
          if (!updated[s.name]) {
            updated[s.name] = { name:s.name, discipline:s.discipline, classification:'field', regions:s.regions, status:'active', role:'treating', phone:'', email:'', notes:'', weeklyVisits:s.totalVisits, uniquePatients:s.uniquePatients };
          } else {
    ...
    // Write visit schedule to Supabase so all users see it
    if (data?.regionData) {
  ...
  const parseCensusFile=(text)=>{
    ...
    function parseCSVLine2(line){const result=[];let cur='';let inQ=false;for(let i=0;i<line.length;i++){if(line[i]==='"'){inQ=!inQ;}else if(line[i]===','&&!inQ){result.push(cur.trim().replace(/^"|"$/g,''));cur='';}else{cur+=line[i];}}result.push(cur.trim().replace(/^"|"$/g,''));return result;}
    ...
    const STATUS_MAP={'active':'active','active - auth pendin':'active_auth_pending','active - auth pending':'active_auth_pending','auth pending':'auth_pending','soc pending':'soc_pending','eval pending':'eval_pending','evaluation pending':'eval_pending','waitlist':'waitlist','on hold':'on_hold','on hold - facility':'on_hold_facility','on hold - pt request':'on_hold_pt','on hold - md request':'on_hold_md','hospitalized':'hospitalized','discharge - change i':'discharge','discharge':'discharge'};
    ...
    for (let i=1;i<rawLines.length;i++){
      ...
      if (region){
        if (!byRegion[region]) byRegion[region]={total:0,activeCensus:0,active:0,active_auth_pending:0,auth_pending:0,soc_pending:0,eval_pending:0,waitlist:0,on_hold:0,on_hold_facility:0,on_hold_pt:0,on_hold_md:0,hospitalized:0,discharge:0,other:0};
  ...
  const totalPatients=hasPariox?(csvData.uniquePatients||0):sum('active_patients',morningReports);
  ...
  return (
    <div style={{ minHeight:'100vh', background:B.bg, color:B.black, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ padding:'24px 28px', maxWidth:1400, margin:'0 auto' }}>
        ...
        {activeTab==='overview'&&(
          <>
            <div style={{ background:`linear-gradient(135deg,${B.darkRed},${B.red},${B.orange})`, borderRadius:16, padding:'18px 28px', marginBottom:20, display:'flex', alignItems:'center', gap:24, flexWrap:'wrap', boxShadow:'0 4px 16px rgba(139,26,16,0.2)', position:'relative', overflow:'hidden' }}>
              ...
              <div style={{ flex:1, minWidth:200, position:'relative' }}>
                <div style={{ fontSize:11, color:'rgba(255,255,255,0.7)', letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:4 }}>📅 Weekly Visit Schedule</div>
                <div style={{ display:'flex', alignItems:'baseline', gap:10 }}>
                  <span style={{ fontSize:44, fontWeight:800, color:'#fff', fontFamily:"'DM Mono',monospace", lineHeight:1 }}>{hasPariox?displayVisits.toLocaleString():'—'}</span>
                  <span style={{ fontSize:16, color:'rgba(255,255,255,0.65)' }}>visits on schedule</span>
                </div>
                {hasPariox&&<div style={{ marginTop:10 }}><div style={{ height:5, background:'rgba(255,255,255,0.2)', borderRadius:3, marginBottom:5 }}><div style={{ height:'100%', width:`${Math.min(displayVisits/CFG.visitTarget*100,100)}%`, background:'#fff', borderRadius:3 }} /></div><div style={{ fontSize:11, color:'rgba(255,255,255,0.65)' }}>{displayVisits>=CFG.visitTarget?`✓ At or above ${CFG.visitTarget} visit target`:`${CFG.visitTarget-displayVisits} below the ${CFG.visitTarget}-visit threshold`}</div></div>}
                {!hasPariox&&<div style={{ fontSize:12, color:'rgba(255,255,255,0.6)', marginTop:6 }}>Upload Pariox weekly export in the Data tab</div>}
              </div>
              {hasPariox&&<div style={{ display:'flex', gap:20, position:'relative' }}>
                {[{label:'Completed',value:totalCompleted},{label:'Missed',value:totalMissed},{label:'Clinicians',value:csvData.staffList?.length||'—'},{label:'Patients',value:csvData.uniquePatients||'—'}].map((s,i)=>(
            ...
            <div style={{ background:B.cardBg, border:`1px solid ${B.border}`, borderRadius:18, padding:'24px 32px', marginBottom:20, display:'flex', alignItems:'center', gap:32, flexWrap:'wrap', boxShadow:'0 1px 6px rgba(139,26,16,0.06)' }}>
              <div style={{ flex:1, minWidth:280 }}>
                ...
                <div style={{ height:8, background:'#F5EDEB', borderRadius:4 }}><div style={{ height:'100%', width:`${visitPct}%`, borderRadius:4, background:visitPct>=100?B.green:`linear-gradient(90deg,${B.darkRed},${B.red},${B.orange})` }} /></div>
                <div style={{ fontSize:11, color:B.gray, marginTop:6 }}>{visitPct}% of target — {visitGap>0?`${visitGap} visits to sustainability`:'🎯 Target reached!'}</div>
              </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:14 }}>
              ...
              <StatCard icon="⚠️" label="Missed Visits" value={totalMissed||0} sub="Cancelled/no-show" color={totalMissed>5?B.danger:B.yellow} alert={totalMissed>5?'Above threshold':null} />
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:20 }}>
              ...
              <StatCard icon="⏰" label="Auths Expiring" value={totalAuthsExpiring||0} sub="Within 7 days" color={totalAuthsExpiring>3?B.danger:B.yellow} alert={totalAuthsExpiring>3?'Action required':null} />
            ...
            {(()=>{
              ...
              const displayTotal=hasCensus&&selectedCensusRegion!=='all'&&censusData.byRegion[selectedCensusRegion]?censusData.byRegion[selectedCensusRegion].total:censusData?.total;
              const displayActive=hasCensus&&selectedCensusRegion!=='all'&&censusData.byRegion[selectedCensusRegion]?censusData.byRegion[selectedCensusRegion].activeCensus:censusData?.activeCensus;
              return (
                <div style={{ marginBottom:20 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12, flexWrap:'wrap', gap:8 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                      ...
                      {hasCensus&&<div style={{ fontSize:11, color:B.lightGray }}>{displayTotal} total · {censusData.lastUpdated} · <span style={{ color:'#0369A1' }}>Live sync</span></div>}
                    </div>
                    <div style={{ display:'flex', gap:5, alignItems:'center', flexWrap:'wrap' }}>
                      {hasCensus&&<span style={{ fontSize:11, color:B.lightGray }}>Region:</span>}
                      {hasCensus&&['all',...regionKeys].map(r=><button key={r} onClick={()=>setSelectedCensusRegion(r)} style={{ padding:'4px 9px', borderRadius:6, border:`1px solid ${selectedCensusRegion===r?B.red:B.border}`, background:selectedCensusRegion===r?'#FFF5F2':'transparent', color:selectedCensusRegion===r?B.red:B.gray, fontSize:11, fontWeight:selectedCensusRegion===r?700:400, cursor:'pointer', fontFamily:'inherit' }}>{r==='all'?'All':r}</button>)}
                  ...
                  {hasCensus?(
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
                      {Object.entries(STATUS_META_OV).map(([key,meta])=>{
                        ...
                        return <div key={key} style={{ background:meta.bg, border:`1px solid ${meta.border}`, borderRadius:12, padding:'12px 14px', position:'relative', overflow:'hidden' }}>
                    ...
                    <div style={{ background:B.cardBg, border:`1px solid ${B.border}`, borderRadius:12, padding:'32px', textAlign:'center', color:B.lightGray }}>
                      <div style={{ fontSize:13 }}>No census data — upload in the <button onClick={()=>setActiveTab('data')} style={{ background:'none', border:'none', color:B.red, cursor:'pointer', fontSize:13, textDecoration:'underline' }}>Data tab</button></div>
                    </div>
                  )}
                </div>
              );
            ...
            <div style={{ background:B.cardBg, border:`1px solid ${B.border}`, borderRadius:16, padding:'20px 24px', boxShadow:'0 1px 6px rgba(139,26,16,0.06)' }}>
              ...
              {totalAuthsExpiring>3&&<AlertItem text={`${totalAuthsExpiring} authorizations expiring within 7 days`} severity="critical" />}
              ...
              {totalMissed>5&&<AlertItem text={`${totalMissed} missed visits — verify reschedule documentation`} severity="warning" />}
              ...
              {activeNotSeen!=null&&activeNotSeen>20&&<AlertItem text={`${activeNotSeen} active patients not scheduled this week`} severity="critical" />}
              {hasCensus&&(censusData.counts.auth_pending||0)+(censusData.counts.active_auth_pending||0)>10&&<AlertItem text={`${(censusData.counts.auth_pending||0)+(censusData.counts.active_auth_pending||0)} patients with auth issues — ~$${(((censusData.counts.auth_pending||0)+(censusData.counts.active_auth_pending||0))*CFG.authRiskVisitsPerWeek*CFG.avgReimbursement/1000).toFixed(1)}K/wk blocked`} severity="warning" />}
            </div>
          </>
        )}
          ...
          <div>
            ...
            <div style={{ background:B.cardBg, border:`1px solid ${B.border}`, borderRadius:16, overflow:'hidden', boxShadow:'0 1px 6px rgba(139,26,16,0.06)' }}>
              <div style={{ display:'grid', gridTemplateColumns:'180px 80px 80px 80px 80px 80px 80px 80px 1fr', padding:'12px 20px', borderBottom:`1px solid ${B.border}`, background:'#FBF7F6' }}>
                {['Coordinator','Patients','Sched','Done','Missed','Auth ⚠','Referrals','Tasks','Completion'].map((h,i)=><div key={i} style={{ fontSize:10, color:B.lightGray, letterSpacing:'0.1em', textTransform:'uppercase', textAlign:i>0&&i<8?'center':'left' }}>{h}</div>)}
              </div>
              {coordinators.length===0?<div style={{ padding:'40px', textAlign:'center', color:B.lightGray }}>No coordinator data yet.</div>:coordinators.map(c=><CoordRow key={c.id} coordinator={c} report={morningReports.find(r=>r.coordinator_id===c.id)} />)}
            </div>
          </div>
        )}
        ...
        {activeTab==='trends'&&(
          <div>
            <div style={{ marginBottom:20 }}><div style={{ fontSize:18, fontWeight:800, marginBottom:4 }}>Visit Trend</div><div style={{ fontSize:13, color:B.gray }}>{csvData?'Showing Pariox import data':'Upload Pariox data in the Data tab'}</div></div>
            <div style={{ background:B.cardBg, border:`1px solid ${B.border}`, borderRadius:16, padding:'24px', marginBottom:20, boxShadow:'0 1px 6px rgba(139,26,16,0.06)' }}>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={trendData} barSize={32}>
                  ...
                  <ReferenceLine y={160} stroke={B.border} strokeDasharray="4 4" />
        ...
        {activeTab==='data'&&(
          <div>
            <div style={{ marginBottom:20 }}><div style={{ fontSize:18, fontWeight:800, marginBottom:4 }}>Data & Integrations</div><div style={{ fontSize:13, color:B.gray }}>Upload Pariox data — syncs to all users via Supabase in real time</div></div>
        ...
        {activeTab==='⚙️'&&(()=>{
          ...
          return (
            <div style={{ maxWidth:720, margin:'0 auto' }}>
              <div style={{ marginBottom:24 }}><div style={{ fontSize:18, fontWeight:800, color:B.black, marginBottom:4 }}>⚙️ Director Settings</div></div>
              {!adminUnlocked?(
                <div style={{ background:B.cardBg, border:`1px solid ${B.border}`, borderRadius:16, padding:'40px', textAlign:'center' }}>
                  <div style={{ fontSize:32, marginBottom:12 }}>🔐</div>
                  <div style={{ fontSize:16, fontWeight:700, color:B.black, marginBottom:24 }}>Enter your 4-digit PIN</div>
                  <div style={{ display:'flex', gap:10, justifyContent:'center' }}>
                    <input type="password" maxLength={4} value={adminPinInput} onChange={e=>{setAdminPinInput(e.target.value);setAdminPinError(false);}} onKeyDown={e=>{if(e.key==='Enter'){if(adminPinInput===CFG.adminPin){setAdminUnlocked(true);setSettingsDraft({...CFG});setAdminPinInput('');}else{setAdminPinError(true);setAdminPinInput('');}}} } placeholder="PIN" style={{ width:100, padding:'12px', textAlign:'center', fontSize:20, letterSpacing:'0.3em', border:`2px solid ${adminPinError?B.danger:B.border}`, borderRadius:10, outline:'none', fontFamily:"'DM Mono',monospace" }} />
                    <button onClick={()=>{if(adminPinInput===CFG.adminPin){setAdminUnlocked(true);setSettingsDraft({...CFG});setAdminPinInput('');}else{setAdminPinError(true);setAdminPinInput('');}} } style={{ background:`linear-gradient(135deg,${B.red},${B.darkRed})`, border:'none', borderRadius:10, color:'#fff', padding:'12px 20px', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>Unlock</button>
                  </div>
                ...
                <div>
                  {[{section:'📅 Visit Targets',fields:[{key:'visitTarget',label:'Weekly Visit Target',type:'number',suffix:'visits/wk'}]},{section:'💰 Revenue',fields:[{key:'revenueTarget',label:'Weekly Revenue Target',type:'number',prefix:'$',suffix:'/week'},{key:'avgReimbursement',label:'Avg Reimbursement Per Visit',type:'number',prefix:'$',suffix:'/visit'},{key:'authRiskVisitsPerWeek',label:'Avg Visits Per Patient / Week',type:'number',suffix:'visits/wk'}]},{section:'👥 Census Targets',fields:[{key:'activeCensusTarget',label:'Active Census Target',type:'number',suffix:'patients'},{key:'coordinatorCap',label:'Coordinator Caseload Cap',type:'number',suffix:'patients'}]},{section:'🔐 Security',fields:[{key:'adminPin',label:'Director PIN',type:'text',maxLength:4}]}].map(section=>(
                    <div key={section.section} style={{ background:B.cardBg, border:`1px solid ${B.border}`, borderRadius:16, padding:'22px 24px', marginBottom:16 }}>
                      <div style={{ fontSize:14, fontWeight:700, color:B.black, marginBottom:16 }}>{section.section}</div>
                      {section.fields.map(field=>(
                        <div key={field.key} style={{ display:'grid', gridTemplateColumns:'1fr 200px', gap:16, alignItems:'center', marginBottom:14 }}>
                          ...
                          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                            ...
                            <input type={field.type} value={draft[field.key]} maxLength={field.maxLength} onChange={e=>setSettingsDraft(prev=>({...prev,[field.key]:field.type==='number'?(parseFloat(e.target.value)||0):e.target.value}))} style={{ flex:1, padding:'10px 12px', border:`1.5px solid ${B.border}`, borderRadius:8, fontSize:14, fontFamily:"'DM Mono',monospace", fontWeight:700, color:B.red, outline:'none', textAlign:'right' }} />
