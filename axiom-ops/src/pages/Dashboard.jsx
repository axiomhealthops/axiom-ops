...
export default function Dashboard() {
  ...
  const [previewRole, setPreviewRole]   = useState(null); // null = own view
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:B.bg, fontFamily:"'DM Sans', sans-serif" }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
        ...
        {/* Top bar */}
        <div style={{ background:B.card, borderBottom:`1px solid ${B.border}`, padding:'10px 24px',
          display:'flex', justifyContent:'space-between', alignItems:'center',
          position:'sticky', top:0, zIndex:40, boxShadow:'0 1px 4px rgba(139,26,16,0.06)', gap:12 }}>

          {/* Left — title + exit preview */}
          ...
          {/* Center — role switcher (super admin only) */}
          {isSuperAdmin && (
              ...
              {ROLE_VIEWS.map(r => {
                ...
                return (
                  <button key={r.role}
                    ...
                    style={{
                      padding:'5px 11px', borderRadius:7, border:'none', fontSize:11, fontWeight: isActive ? 700 : 500,
